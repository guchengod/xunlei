...

{
  ...
  "dir":   "电影/2025(可选)",           // 下载根目录下的相对目录, 不存在会自动创建
...
内部实际调用的是引擎的 `/drive/v1/task` / `/drive/v1/tasks` 等接口:
创建任务请求体为
`{type:"user#download-url", name, file_name, file_size:"0", space, params:{target, url, ...}}`。
