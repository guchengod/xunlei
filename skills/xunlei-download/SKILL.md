...
脚本直接返回引擎的原始 JSON；添加磁力时服务端会先按迅雷面板流程解析文件树，再默认选择全部文件创建任务。任一异常（服务不可达 / 未登录 / 解析失败 / 校验失败）会打印错误并返回非零退出码。
...
指定的 `--dir`/`--path` 应来自 `dirs` 返回的目录树；服务端会像面板一样逐级解析真实目录 ID，目录不存在或尚未被迅雷索引时会拒绝创建任务。
...
| `dir` | 否 | 下载根目录下的相对目录，如 `电影` / `动漫/2025`（应来自 `dirs`） |
